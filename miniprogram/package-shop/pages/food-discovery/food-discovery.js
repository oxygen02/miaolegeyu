...
Page({
  ...
  // 执行搜索
  onSearch() {
    const { searchKeyword, allShops } = this.data;
    if (!searchKeyword.trim()) {
      this.setData({ shops: allShops });
      return;
    }
    const keyword = searchKeyword.trim().toLowerCase();
    const filtered = allShops.filter(shop => {
      return (shop.name && shop.name.toLowerCase().includes(keyword)) ||
             (shop.cuisineName && shop.cuisineName.includes(keyword)) ||
             (shop.location && shop.location.includes(keyword));
    });
  ...
  onShopItemTap(e) {
    const { id } = e.currentTarget.dataset;
    wx.navigateTo({
      url: `/package-shop/pages/shop-detail/shop-detail?id=${id}`
    });
  },
      ...
      if (result.success && result.count > 0) {
        ...
        wx.showModal({
          ...
          success: (res) => {
            if (res.confirm) {
              wx.navigateTo({
                url: `/package-shop/pages/shop-detail/shop-detail?id=${shop._id}&openAppointment=1&notifyInterested=1`
              });
            }
          }
        });
        ...
        // 没有意向用户，直接跳转
        wx.navigateTo({
          url: `/package-shop/pages/shop-detail/shop-detail?id=${shop._id}&openAppointment=1`
        });
      }
    } catch (err) {
      // 查询失败，直接跳转
  ...
  onUploadTap() {
    wx.navigateTo({
      url: '/package-shop/pages/upload-shop/upload-shop'
    });
  },
  ...
  onShareAppMessage() {
    return {
      title: '新店探索 - 喵了个鱼',
      path: '/package-shop/pages/food-discovery/food-discovery'
    };
  }
});
