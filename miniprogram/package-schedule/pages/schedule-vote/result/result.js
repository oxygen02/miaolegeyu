...
Page({
  ...
  goFill() {
    wx.navigateTo({
      url: `/package-schedule/pages/schedule-vote/fill/fill?voteId=${this.data.voteId}&title=${encodeURIComponent(this.data.vote?.title || '')}`
    });
  },
            ...
            wx.showModal({
              ...
              success: (createRes) => {
                if (createRes.confirm) {
                  ...
                  wx.navigateTo({
                    url: `/package-create/pages/create-mode-a/create-mode-a?${params}`
                  });
                } else {
